...
export function DataTable<T extends Record<string, any>>({
  ...
  if (loading) {
    return (
      <div className={cn("w-full bg-background border border-border", className)}>
        <div className="animate-pulse p-6">
          {searchable && <div className="mb-6 h-11 bg-muted rounded"></div>}
          <div className="border border-border overflow-hidden">
            <div className="bg-muted/30 h-14"></div>
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="h-14 border-t border-border bg-background"></div>
            ))}
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 border-b border-border">
          <div className="relative w-full sm:w-auto sm:flex-1 sm:max-w-sm">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-border bg-background text-sm focus:outline-none focus:border-foreground transition-all"
            />
          </div>
        </div>
      )}
                    ...
                    {column.filterable && (
                      <div className="mt-3 relative">
                        <input
                          ...
                          className="w-full px-3 py-1.5 text-xs border border-border bg-background focus:outline-none focus:border-foreground transition-all"
                        />
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-background border-t border-border">
          ...
          <div className="flex items-center gap-2 order-1 sm:order-2">
            <button
              ...
              className="px-3 py-2 text-sm border border-border hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              Previous
            </button>
              ...
              {Array.from({ length: Math.min(totalPages, 5) }).map((_, i) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "px-3 py-2 text-sm border border-border hover:bg-muted transition-colors",
